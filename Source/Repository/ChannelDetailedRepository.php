...
class ChannelDetailedRepository extends AbstractGetableRepository
{
    ...
    {
        $sql = "SELECT id, name, description, email, (SELECT COUNT(*) FROM SUBSCRIBE S WHERE S.channelId = C.id) AS subCount FROM " . static::getNomTable() . " C WHERE ";
        $values = [];
    ...
    {
        $sql = "SELECT id, name, description, email, (SELECT COUNT(*) FROM SUBSCRIBE S WHERE S.channelId = C.id) AS subCount FROM " . static::getNomTable() . " C WHERE " . static::getNomClePrimaire() . "=:Tag";
